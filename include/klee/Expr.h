...
public:
  UpdateNode(const UpdateNode *_next, const ref<Expr> &_index,
...
public:
  ...
  static ref<ConstantExpr> alloc(const llvm::APInt &v) {
    ...
    if (v.sge(-100) && v.ule(100) && v.getBitWidth() == Expr::Int64) {
      CachedExpr = &SmallConstants[v.getSExtValue() + 100];
    }

    if (CachedExpr != nullptr && !CachedExpr->isNull()) {
      return *CachedExpr;
    } else {
      ref<ConstantExpr> r(new ConstantExpr(v));
      r->computeHash();
      if (CachedExpr != nullptr)
        *CachedExpr = r;
      return r;
    }
  }
